#include "K_game.h"

#define ACTOR(ident)                                                                                                   \
    extern const ActorTable TAB_##ident;                                                                               \
    ACTORS[ACT_##ident] = &TAB_##ident;

void POPULATE_ACTORS_TABLE() {
    extern const ActorTable* ACTORS[ACT_SIZE];

    static const ActorTable TAB_NULL = {NULL};
    ACTORS[ACT_NULL] = &TAB_NULL;

    ACTOR(PLAYER_SPAWN);
    ACTOR(PLAYER);
    ACTOR(PLAYER_EFFECT);
    ACTOR(PLAYER_DEAD);
    ACTOR(POINTS);
    ACTOR(WARP);
    ACTOR(CHECKPOINT);
    ACTOR(CHECKPOINT_EFFECT);
    ACTOR(GOAL_BAR);
    ACTOR(GOAL_MARK);
    ACTOR(PLATFORM);
    ACTOR(PLATFORM_TURN);
    ACTOR(WATER);
    ACTOR(WATER_TRIGGER);
    ACTOR(AUTOSCROLL);
    ACTOR(BOUNDS);
    ACTOR(BUSH);
    ACTOR(CLOUD);
    ACTOR(CLOUDS);
    ACTOR(COIN);
    ACTOR(COIN_POP);
    ACTOR(WATER_SPLASH);
    ACTOR(BUBBLE);
    ACTOR(BLOCK);
    ACTOR(BLOCK_BUMP);
    ACTOR(BRICK_SHARD);
    ACTOR(SUPER_MUSHROOM);
    ACTOR(FIRE_FLOWER);
    ACTOR(STARMAN);
    ACTOR(1UP_MUSHROOM);
    ACTOR(POISON_MUSHROOM);
    ACTOR(GREEN_LUI);
    ACTOR(BEETROOT);
    ACTOR(FIREBALL_PROJECTILE);
    ACTOR(BEETROOT_PROJECTILE);
    ACTOR(EXPLODE);
    ACTOR(GOOMBA);
    ACTOR(DEAD);
    ACTOR(KOOPA);
    ACTOR(KOOPA_SHELL);
    ACTOR(PIRANHA_PLANT);
    ACTOR(LAMP_LIGHT);
    ACTOR(TUBE_BUBBLES);
    ACTOR(TUBE_BUBBLE);
    ACTOR(WATERFALL);
    ACTOR(GOOMBA_PARTY);
    ACTOR(LAVA);
    ACTOR(LAVAFALL);
    ACTOR(LAVA_BUBBLER);
    ACTOR(LAVA_BUBBLE);
    ACTOR(ROTODISC_BALL);
    ACTOR(ROTODISC);
    ACTOR(BOWSER);
    ACTOR(BOWSER_DEAD);
    ACTOR(BOWSER_FIRE_PROJECTILE);
    ACTOR(LAVA_WAVER);
    ACTOR(BOWSER_EFFECT);
    ACTOR(SINK);
    ACTOR(SINK_BUBBLE);
    ACTOR(CHEEP_SPAWNER);
    ACTOR(CHEEP);
    ACTOR(CHEEP_BLUE);
    ACTOR(SPRING);
    ACTOR(BRO);
    ACTOR(BRO_LAYER);
    ACTOR(HAMMER_PROJECTILE);
    ACTOR(SILVER_HAMMER_PROJECTILE);
    ACTOR(CLOUD_FACE);
    ACTOR(PODOBOO);
    ACTOR(LAVA_SPLASH);
    ACTOR(STARLAND_LIGHT);
    ACTOR(STARLAND_GLOW);
    ACTOR(SPINY);
    ACTOR(STAR);
    ACTOR(SHOOTING_STAR_SPAWNER);
    ACTOR(SHOOTING_STAR);
    ACTOR(FAKE_BRICK);
    ACTOR(LAKITU);
    ACTOR(SPINY_EGG);
    ACTOR(BILL_BLASTER);
    ACTOR(BULLET_BILL);
    ACTOR(PIRANHA_HEAD);
    ACTOR(PARATROOPA);
    ACTOR(CHEEP_SPIKY);
    ACTOR(THWOMP);
    ACTOR(BOUNCY_TREE);
    ACTOR(ELECTRIC_CORAL);
    ACTOR(EXPLODE2);
    ACTOR(SECRET_TEXT);

    ACTOR(BUZZY);
    ACTOR(BUZZY_SHELL);
    ACTOR(PSWITCH);
    ACTOR(PSWITCH_BLOCK);
    ACTOR(PSWITCH_COIN);
    ACTOR(HAMMER_SUIT);
    ACTOR(SHOTGUN);
    ACTOR(FLOWER);
    ACTOR(BULLET_PROJECTILE);
}
