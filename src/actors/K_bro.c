...
static void tick(GameActor* actor) {
	...
	if (VAL(actor, BRO_MOVE) < 0L) {
		++VAL(actor, BRO_MOVE);
		move_actor(actor, POS_ADD(actor, FxFrom(-2L), FxZero));
	...
	if (ANY_FLAG(actor, FLG_BRO_JUMP)) {
		if (VAL(actor, BRO_JUMP) == 5L && ANY_FLAG(actor, FLG_BRO_BOTTOM | FLG_BRO_TOP)) {
			...
			VAL(actor, BRO_DOWN) = actor->pos.y + FxFrom(80L);
	...
	if (VAL(actor, BRO_THROW_STATE) > 0L) {
		...
		if (VAL(actor, BRO_THROW_STATE) > 30L) {
			...
			GameActor* missile = create_actor(VAL(actor, BRO_MISSILE),
				POS_ADD(actor, ANY_FLAG(actor, FLG_X_FLIP) ? FxFrom(-9L) : FxFrom(9L), FxFrom(-20L)));
			if (missile != NULL)
				switch (missile->type) {
				default: {
					VAL(missile, X_SPEED)
						= FxFrom((ANY_FLAG(actor, FLG_X_FLIP) ? -1L : 1L) * (1L + rng(5L)));
					VAL(missile, Y_SPEED) = FxFrom(-6L - rng(5L));
			...
			|| (VAL(actor, BRO_JUMP_STATE) == 2L && actor->pos.y > VAL(actor, BRO_DOWN))))
	{
		...
		const Fixed jut
			= FxFrom((Sint32)(VAL(actor, Y_SPEED) > FxZero) - (Sint32)(VAL(actor, Y_SPEED) < FxZero));
...
static void draw(const GameActor* actor) {
	...
	draw_actor(actor, fmt(tex, (int)((float)game_state.time / 7.142857142857143f) % 2L), 0, B_WHITE, FALSE);
}

static void draw_corpse(const GameActor* actor) {
	draw_actor(actor, "enemies/bro0", 0.f, B_WHITE, FALSE);
}
...
static void collide(GameActor* actor, GameActor* from) {
	...
	case ACT_PLAYER: {
		if (check_stomp(actor, from, FxFrom(-16L), 200L))
