...
static void tick(GameActor* actor) {
	...
	move_enemy(actor, (FVec2){red ? FxFrom(2L) : FxOne, 19005L}, red);
}
...
static void draw_corpse(const GameActor* actor) {
	draw_actor(
		actor, ANY_FLAG(actor, FLG_KOOPA_RED) ? "enemies/shell_red0" : "enemies/shell0", 0.f, B_WHITE, FALSE);
}
...
static void collide_shell(GameActor* actor, GameActor* from) {
	...
	case ACT_PLAYER: {
		...
		if (!ANY_FLAG(actor, FLG_SHELL_ACTIVE)) {
			...
			VAL(actor, X_SPEED) = (from->pos.x > actor->pos.x) ? FxFrom(-6L) : FxFrom(6L);
...
static void tick_parakoopa(GameActor* actor) {
	if (!ANY_FLAG(actor, FLG_PARAKOOPA_START)) {
		...
		VAL(actor, PARAKOOPA_ANGLE) = FxRad(FxFrom(rng(360L)));
		FLAG_ON(actor, FLG_PARAKOOPA_START);
	}

	move_actor(actor,
		(FVec2){actor->pos.x, VAL(actor, PARAKOOPA_Y) + Fmul(FxFrom(50L), Fcos(VAL(actor, PARAKOOPA_ANGLE)))});
	VAL(actor, PARAKOOPA_ANGLE) += FxRad(FxFrom(2L));
