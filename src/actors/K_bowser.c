...
static void create(GameActor* actor) {
	actor->box.start.x = FxFrom(-30L);
	actor->box.start.y = FxFrom(-70L);
...
static void tick(GameActor* actor) {
	...
	if (nearest != NULL) {
		...
		if (autoscroll == NULL && nearest->pos.x > (game_state.size.x - FxFrom(952L))) {
			autoscroll
	...
	if (VAL(actor, BOWSER_C) > 0L) {
		VAL(actor, X_SPEED) = ANY_FLAG(actor, FLG_BOWSER_1) ? FxFrom(2L) : FxFrom(-2L);
		--VAL(actor, BOWSER_C);
	} else
		VAL(actor, X_SPEED) = FxZero;

	// 891, 892
	if ((!ANY_FLAG(actor, FLG_BOWSER_1) && (actor->pos.x + actor->box.start.x) < (game_state.size.x - FxFrom(560L)))
		|| (ANY_FLAG(actor, FLG_BOWSER_1)
			&& (actor->pos.x + actor->box.end.x) > (game_state.size.x - FxFrom(80L))))
			...
			|| (ANY_FLAG(actor, FLG_BOWSER_ANIM_FIRE_START2) && VAL(actor, BOWSER_FRAME) >= 6800L)))
	{
		...
		if (VAL(actor, BOWSER_K) < 5L) {
			GameActor* napalm = create_actor(ACT_MISSILE_NAPALM,
				POS_ADD(actor, ANY_FLAG(actor, FLG_X_FLIP) ? FxFrom(-18L) : FxFrom(18L), FxFrom(-32L)));
			if (napalm != NULL) {
				FLAG_ON(napalm, (actor->flags & FLG_X_FLIP) | FLG_MISSILE_SHIMMY);
				VAL(napalm, X_SPEED) = Fmul(ANY_FLAG(napalm, FLG_X_FLIP) ? FxFrom(-4L) : FxFrom(4L),
					VAL(actor, BOWSER_MISSILE_SPEED));
				VAL(napalm, MISSILE_HEIGHT) = VAL(actor, BOWSER_HEIGHT) - FxFrom(28L + (rng(3L) * 36L));
			}
			...
			const FVec2 mpos
				= POS_ADD(actor, (mflags & FLG_X_FLIP) ? FxFrom(-18L) : FxFrom(18L), FxFrom(-32L));
			const Fixed mspd = Fmul(
				(mflags & FLG_X_FLIP) ? FxFrom(-4L) : FxFrom(4L), VAL(actor, BOWSER_MISSILE_SPEED));
			for (Fixed i = 0L; i < 3L; i++) {
				...
				if (napalm != NULL) {
					...
					VAL(napalm, MISSILE_HEIGHT)
						= VAL(actor, BOWSER_HEIGHT) - FxFrom(28L + (i * 36L));
				}
			}
	...
	if (ALL_FLAG(actor, FLG_BOWSER_ACTIVE | FLG_BOWSER_GUN) && VAL(actor, BOWSER_GUN)++ > 100L) {
		GameActor* bullet = create_actor(ACT_BULLET_BILL,
			POS_ADD(actor, ANY_FLAG(actor, FLG_X_FLIP) ? FxFrom(-39L) : FxFrom(39L), FxFrom(-23L)));
		if (bullet != NULL) {
			bullet->depth = FxHalf;
	...
	if (slipper == NULL)
		...
		if (VAL(actor, BOWSER_SLIP) == 0L)
			...
			const Fixed slip = FxFrom(VAL(actor, BOWSER_SLIP));
...
static void collide(GameActor* actor, GameActor* from) {
	...
	case ACT_PLAYER: {
		if (VAL(actor, BOWSER_HURT) >= (VAL(actor, BOWSER_HURT_TIME) - 5L)
			&& from->pos.y < (actor->pos.y - FxFrom(45L)))
			break;

		if (VAL(actor, BOWSER_HURT) > 0L || from->pos.y >= (actor->pos.y - FxFrom(45L))
...
static void tick_corpse(GameActor* actor) {
	if (actor->pos.y > (game_state.bounds.end.y + FxFrom(100L)) && game_state.sequence.type == SEQ_BOWSER_END) {
	...
	if (ANY_FLAG(actor, FLG_KUPPA_LAVA_LOVE)) {
		...
		if (VAL(actor, KUPPA_PFRBRBR) < 50L) {
			GameActor* bubble
				= create_actor(ACT_LAVA_BUBBLE, POS_ADD(actor, FxFrom(-23L + rng(47L)), FxFrom(-16L)));
			if (bubble != NULL) {
...
static void create_laver(GameActor* actor) {
	...
	actor->box.start.y = FxFrom(-17L);
	actor->box.end.x = FxFrom(17L);
	actor->box.end.y = FxFrom(15L);

	VAL(actor, KUPPA_LAVER) = FxFrom(16L);
}
...
static void tick_laver(GameActor* actor) {
	...
	move_actor(actor, POS_ADD(actor, ANY_FLAG(actor, FLG_X_FLIP) ? FxFrom(-4L) : FxFrom(4L), FxZero));
	collide_actor(actor);
}
