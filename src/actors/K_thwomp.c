...
static void draw(const GameActor* actor) {
	const char* tex = NULL;
	if (ANY_FLAG(actor, FLG_THWOMP_LAUGH))
		tex = fmt("enemies/thwomp_laugh%u", Fx2Int(VAL(actor, THWOMP_FRAME)) % 3L);
	else
		switch (Fx2Int(VAL(actor, THWOMP_FRAME)) % 7L) {
		default:
			tex = "enemies/thwomp0";
			break;
		case 1L:
...
static void draw_corpse(const GameActor* actor) {
	draw_actor(actor, "enemies/thwomp0", 0.f, B_WHITE, FALSE);
}
