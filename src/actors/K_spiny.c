...
static void draw_corpse(const GameActor* actor) {
	draw_actor(
		actor, ANY_FLAG(actor, FLG_SPINY_GRAY) ? "enemies/spiny_gray1" : "enemies/spiny1", 0.f, B_WHITE, FALSE);
}
...
static void draw_egg(const GameActor* actor) {
	if (ANY_FLAG(actor, FLG_SPINY_GREEN)) {
		draw_actor(actor, "enemies/spiny_egg_green", Fx2Float(VAL(actor, SPINY_ANGLE)), B_WHITE, FALSE);
		return;
	}

	const char* tex = ANY_FLAG(actor, FLG_SPINY_HATCH) ? fmt("enemies/spiny_egg%u", 1L + VAL(actor, SPINY_FRAME))
	                                                   : "enemies/spiny_egg0";
	draw_actor(actor, tex, Fx2Float(VAL(actor, SPINY_ANGLE)), B_WHITE, FALSE);
}

static void draw_egg_corpse(const GameActor* actor) {
	if (ANY_FLAG(actor, FLG_SPINY_GREEN))
		draw_actor_offset(actor, "enemies/spiny_egg_green", B_XY(0.f, 31.f), 0.f, B_WHITE, FALSE);
	else
		draw_actor(actor, "enemies/spiny1", 0.f, B_WHITE, FALSE);
}
