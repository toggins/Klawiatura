...
static void tick_bar(GameActor* actor) {
	...
	if ((VAL(actor, Y_SPEED) > FxZero && actor->pos.y >= (VAL(actor, GOAL_Y) + FxFrom(220L)))
...
static void draw_bar(const GameActor* actor) {
	draw_actor(actor, "markers/goal_bar", 0.f, B_WHITE, FALSE);
}
...
static void collide_bar(GameActor* actor, GameActor* from) {
	...
	if (ANY_FLAG(actor, FLG_GOAL_JACKPOT)) {
		...
		if (actor->pos.y < (gy + FxFrom(30L)))
			points = 10000L;
		else if (actor->pos.y < (gy + FxFrom(60L)))
			points = 5000L;
		else if (actor->pos.y < (gy + FxFrom(100L)))
			points = 2000L;
		else if (actor->pos.y < (gy + FxFrom(150L)))
			points = 1000L;
		else if (actor->pos.y < (gy + FxFrom(200L)))
			points = 500L;
	}
	give_points(actor, get_owner(from), points);
	win_player(from);

	GameActor* bar = create_actor(ACT_GOAL_BAR_FLY, POS_ADD(actor, FxFrom(-2L), FxFrom(7L)));
	if (bar != NULL) {
		const Fixed dir = 154416L + (-12868L + Fmul(FxFrom(rng(3L)), 12868L));
...
static void create_fly(GameActor* actor) {
	actor->box.start.x = actor->box.start.y = FxFrom(-23L);
	actor->box.end.x = actor->box.end.y = FxFrom(23L);
}
...
static void draw_fly(const GameActor* actor) {
	draw_actor(actor, "markers/goal_bar2", Fx2Float(VAL(actor, GOAL_ANGLE)), B_WHITE, FALSE);
}
