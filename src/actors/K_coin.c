...
static void load() {
	load_texture_num("items/coin%u", 3L, FALSE);
...
static void create(GameActor* actor) {
	actor->box.start.x = FxFrom(6L);
	actor->box.start.y = FxFrom(2L);
	actor->box.end.x = FxFrom(25L);
	actor->box.end.y = FxFrom(30L);
...
static void collide(GameActor* actor, GameActor* from) {
	...
	case ACT_BLOCK_BUMP: {
		...
		GameActor* pop = create_actor(ACT_COIN_POP, POS_ADD(actor, FxFrom(16L), FxFrom(28L)));
		if (pop != NULL)
...
static void load_pop() {
	load_texture_num("items/coin_pop%u", 4L, FALSE);
	load_texture_num("effects/spark%u", 4L, FALSE);
...
static void draw_pop(const GameActor* actor) {
	...
	if (ANY_FLAG(actor, FLG_COIN_POP_SPARK))
		...
		const Uint8 frame = (VAL(actor, COIN_POP_FRAME) / 100L) % 5L;
