...
static void draw_ball(const GameActor* actor) {
	draw_actor(actor, "enemies/rotodisc_ball", 0.f, B_WHITE, FALSE);
}
...
static void tick(GameActor* actor) {
	...
	if (ANY_FLAG(actor, FLG_ROTODISC_FLOWER)) {
		VAL(actor, ROTODISC_LENGTH) += ANY_FLAG(actor, FLG_ROTODISC_FLOWER2) ? FxFrom(-5L) : FxFrom(5L);
		if (VAL(actor, ROTODISC_LENGTH) >= FxFrom(200L) && !ANY_FLAG(actor, FLG_ROTODISC_FLOWER2))
...
static void draw(const GameActor* actor) {
	draw_actor(actor, fmt("enemies/rotodisc%u", game_state.time % 26L), 0.f, B_WHITE, FALSE);
}
