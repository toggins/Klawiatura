...
static Bool hit_player(struct GameObject* pawn) {
    if (state.sequence.type == SEQ_WIN || pawn->values[VAL_PLAYER_FLASH] > 0L ||
        pawn->values[VAL_PLAYER_STARMAN] > 0L || pawn->values[VAL_PLAYER_WARP] > 0L)
        return false;
...
static struct GameObject* kill_enemy(struct GameObject* enemy, Bool kick) {
    switch (enemy->type) {
        ...
        case OBJ_BULLET_BILL: {
            enemy->values[VAL_X_SPEED] = Fmul(enemy->values[VAL_X_SPEED], 0x0000B13B);
            enemy->values[VAL_Y_SPEED] = Fmax(enemy->values[VAL_Y_SPEED], FxZero);
            enemy->flags |= FLG_BULLET_DEAD;
    ...
    struct GameObject* dead = get_object(create_object(OBJ_DEAD, POS_ADD(enemy, FxZero, FfInt(-32L))));
    if (dead != NULL) {
    ...
    switch (self->type) {
        ...
        case OBJ_PLAYER: {
            ...
            switch (other->type) {
                ...
                case OBJ_PLAYER: {
                    if (object_is_alive((ObjectID)(other->values[VAL_PLAYER_WARP])))
                        break;
        ...
        case OBJ_WARP: {
            struct GameObject* other = &(state.objects[other_id]);
            if (other->type != OBJ_PLAYER || object_is_alive((ObjectID)(other->values[VAL_PLAYER_WARP])))
                break;

            struct GamePlayer* player = get_player((PlayerID)(other->values[VAL_PLAYER_INDEX]));
            if (player == NULL)
                break;

            Bool warped = false;
            switch (self->values[VAL_WARP_ANGLE]) {
                case 0: {
                    if (other->values[VAL_X_TOUCH] > 0L && (player->input & GI_RIGHT)) {
                        move_object(other_id, POS_ADD(self, Fsub(self->bbox[1][0], other->bbox[1][0]), -FxOne));
                        warped = true;
                    }
                    break;
                }
                ...
                case 1: {
                    if (other->values[VAL_Y_TOUCH] < 0L && (player->input & GI_UP)) {
                        move_object(other_id, POS_ADD(self, FxZero, Fsub(other->bbox[0][1], self->bbox[0][1])));
                        warped = true;
                    }
                    break;
                }
                ...
                case 2: {
                    if (other->values[VAL_X_TOUCH] < 0L && (player->input & GI_LEFT)) {
                        move_object(other_id, POS_ADD(self, Fsub(self->bbox[0][0], other->bbox[0][0]), -FxOne));
                        warped = true;
                    }
                    break;
                }
                ...
                default: {
                    if (other->values[VAL_PLAYER_GROUND] > 0L && (player->input & GI_DOWN)) {
                        move_object(other_id, self->pos);
                        warped = true;
                    }
                    break;
                }
            ...
            if (warped) {
                other->depth = FfInt(21L);
                other->values[VAL_X_SPEED] = FxZero;
                other->values[VAL_Y_SPEED] = FxZero;
                other->values[VAL_PLAYER_WARP] = self_id;
                other->values[VAL_PLAYER_WARP_STATE] = 0L;
            ...
            while (object_is_alive(oid)) {
                ...
                if (bid != oid && other->values[VAL_SPROUT] <= 0L && !(other->flags & FLG_DESTROY)) {
                    ...
                    if (x1 < ox2 && x2 > ox1 && y1 < oy2 && y2 > oy1 &&
                        (!bump_check(oid, bid) || (object->flags & FLG_DESTROY)))
                        return;
                }
                oid = next;
            }
...
static PlayerFrames get_player_frame(const struct GameObject* object) {
    const struct GameObject* warp = get_object((ObjectID)(object->values[VAL_PLAYER_WARP]));
    if (warp != NULL)
        ...
        if (object->values[VAL_SPROUT] > 0L)
            ...
            switch (object->type) {
                ...
                case OBJ_PLAYER: {
                    ...
                    struct GameObject* warp = get_object((ObjectID)(object->values[VAL_PLAYER_WARP]));
                    if (warp != NULL) {
                        if (object->flags & FLG_PLAYER_WARP_OUT) {
                            switch (warp->values[VAL_WARP_ANGLE_OUT]) {
                                case 0: {
                                    move_object(oid, POS_ADD(object, -FxOne, FxZero));
                                    object->values[VAL_PLAYER_FRAME] = Fadd(object->values[VAL_PLAYER_FRAME], 0x1EB8);
                            ...
                            if (!touching_solid(HITBOX(object))) {
                                ...
                                object->values[VAL_PLAYER_WARP_STATE] = 0L;
                                object->flags &= ~FLG_PLAYER_WARP_OUT;
                            }
                        } else {
                            switch (warp->values[VAL_WARP_ANGLE]) {
                                case 0: {
                            ...
                            ++object->values[VAL_PLAYER_WARP_STATE];
                            if (object->values[VAL_PLAYER_WARP_STATE] > 60L) {
                                if (warp->values[VAL_WARP_BOUNDS_X1] != warp->values[VAL_WARP_BOUNDS_X2] &&
                                    warp->values[VAL_WARP_BOUNDS_Y1] != warp->values[VAL_WARP_BOUNDS_Y2]) {
                                    player->bounds[0][0] = warp->values[VAL_WARP_BOUNDS_X1];
                                    player->bounds[0][1] = warp->values[VAL_WARP_BOUNDS_Y1];
                                    player->bounds[1][0] = warp->values[VAL_WARP_BOUNDS_X2];
                                    player->bounds[1][1] = warp->values[VAL_WARP_BOUNDS_Y2];
                                }

                                move_object(
                                    oid, (fvec2){warp->values[VAL_WARP_X], Fadd(warp->values[VAL_WARP_Y], FfInt(50L))}
                                );
                                object->flags |= FLG_PLAYER_WARP_OUT;

                                skip_interp(oid);
                                play_sound_at_object(object, "WARP");
                            }
                        }
                ...
                case OBJ_MISSILE_HAMMER: {
                    ...
                    const PlayerID player = get_owner_id((ObjectID)(object->values[VAL_MISSILE_OWNER]));
                    if ((player >= 0L && !in_player_view(object, player, FxZero, true)) ||
                        (player < 0L && !in_any_view(object, FfInt(128L), true)))
                        object->flags |= FLG_DESTROY;
                    break;
                }
                ...
                case OBJ_MISSILE_SILVER_HAMMER: {
                    object->values[VAL_MISSILE_ANGLE] = Fadd(
                        object->values[VAL_MISSILE_ANGLE],
                        object->values[VAL_X_SPEED] < FxZero ? -0x0000B2B9 : 0x0000B2B9
                    );
                    object->values[VAL_Y_SPEED] = Fadd(object->values[VAL_Y_SPEED], 0x00006666);
                    move_object(oid, POS_SPEED(object));
                    ...
                    if (object->values[VAL_MISSILE_HITS] > 0L) {
                        ...
                        struct BlockList list = {0};
                        list_block_at(
                            &list, (fvec2[2]){{Fadd(object->pos[0], object->bbox[0][0]),
                                               Fadd(object->pos[1], object->bbox[0][1])},
                        ...
                        if (hit > 0L) {
                            ...
                            object->values[VAL_Y_SPEED] = (hit > 1L) ? -(object->values[VAL_Y_SPEED]) : FfInt(-5L);
                        ...
                        for (size_t i = 0; i < list.num_objects; i++) {
                            ...
                            if (is_solid(bid, false, true)) {
                                ...
                                switch (bumped->type) {
                                    ...
                                    case OBJ_BULLET_BILL: {
                                        give_points(
                                            bumped, get_owner_id((ObjectID)(object->values[VAL_MISSILE_OWNER])), 100L
                                        );
                ...
                case OBJ_BILL_BLASTER: {
                    ...
                    if (nearest != NULL && nearest->pos[0] > Fsub(object->pos[0], FfInt(64L)) &&
                        nearest->pos[0] < Fadd(object->pos[0], FfInt(96L)))
                        object->flags |= FLG_BLASTER_BLOCKED;

                    if (!(object->flags & FLG_BLASTER_BLOCKED) && in_any_view(object, FxZero, false))
                        ++(object->values[VAL_BLASTER_TIME]);

                    if (object->values[VAL_BLASTER_TIME] > 25L) {
                        Bool flip = (nearest == NULL || nearest->pos[0] < Fadd(object->pos[0], FfInt(16L)));
                        struct GameObject* bullet = get_object(
                            create_object(OBJ_BULLET_BILL, POS_ADD(object, flip ? FxZero : FfInt(32L), FfInt(16L)))
                        );
                        if (bullet != NULL) {
                            bullet->values[VAL_X_SPEED] = (state.flags & GF_FUNNY) ? 0x00068000 : 0x00034000;
                            if (flip) {
                                bullet->values[VAL_X_SPEED] = Fmul(bullet->values[VAL_X_SPEED], -FxOne);
                                bullet->flags |= FLG_X_FLIP;
                            }

                            create_object(OBJ_EXPLODE, bullet->pos);
                            play_sound_at_object(bullet, "BANG1");
                        }

                        object->values[VAL_BLASTER_TIME] = (-50L - (random() % 150L));
                    }
                    break;
                }
                ...
                case OBJ_BULLET_BILL: {
                    ...
                    if (object->flags & FLG_BULLET_DEAD) {
                        object->values[VAL_Y_SPEED] = Fadd(object->values[VAL_Y_SPEED], 0x00006666);
                ...
                case OBJ_SPIKE_CANNON: {
                    if ((state.time % ((state.flags & GF_FUNNY) ? 20L : 50L)) == 0L &&
                        in_any_view(object, FxZero, false))
                        ++(object->values[VAL_SPIKE_BALL_TIME]);

                    if (object->values[VAL_SPIKE_BALL_TIME] >= 2L) {
                        struct GameObject* ball =
                            get_object(create_object(OBJ_SPIKE_BALL, POS_ADD(object, FfInt(32L), FfInt(22L))));
                        if (ball != NULL) {
                            ball->values[VAL_X_SPEED] = FfInt(-2L + (random() % 5L));
                            if (!(object->flags & FLG_Y_FLIP))
                                ball->values[VAL_Y_SPEED] = FfInt(-7L - (random() % 5L));

                            create_object(OBJ_EXPLODE, ball->pos);
                            play_sound_at_object(ball, "BANG2");
                        }

                        object->values[VAL_SPIKE_BALL_TIME] = 0L;
                    }
                    break;
                }
                ...
                case OBJ_SPIKE_BALL: {
                    ...
                    move_object(oid, POS_SPEED(object));
                    object->values[VAL_Y_SPEED] = Fadd(object->values[VAL_Y_SPEED], 0x0000199A);
                    object->values[VAL_SPIKE_BALL_ANGLE] =
                        Fadd(object->values[VAL_SPIKE_BALL_ANGLE], Frad(FfInt(120L)));

                    const ObjectID eid = create_object(OBJ_SPIKE_BALL_EFFECT, object->pos);
                    struct GameObject* effect = get_object(eid);
                    if (effect != NULL) {
                        interp[eid].from[0] = interp[oid].pos[0];
                        interp[eid].from[1] = interp[oid].pos[1];
                ...
                case OBJ_SPIKE_BALL_EFFECT: {
                    object->depth = Fadd(object->depth, 0x00000FFF);

                    object->values[VAL_SPIKE_BALL_ANGLE] =
                        Fadd(object->values[VAL_SPIKE_BALL_ANGLE], Frad(FfInt(120L)));
                    object->values[VAL_SPIKE_BALL_ALPHA] = Fsub(object->values[VAL_SPIKE_BALL_ALPHA], FfInt(5L));
                    if (object->values[VAL_SPIKE_BALL_ALPHA] <= FxZero)
                        object->flags |= FLG_DESTROY;
                    break;
                }
                ...
                case OBJ_PARAKOOPA: {
                    if (!(object->flags & FLG_PARAKOOPA_START)) {
                        ...
                        object->values[VAL_PARAKOOPA_ANGLE] = Frad(FfInt(random() % 360L));
                        object->flags |= FLG_PARAKOOPA_START;
                    }

                    move_object(
                        oid, (fvec2){object->pos[0], Fadd(
                                                         object->values[VAL_PARAKOOPA_Y],
                                                         Fmul(FfInt(50L), Fcos(object->values[VAL_PARAKOOPA_ANGLE]))
                                                     )}
                    );
                    object->values[VAL_PARAKOOPA_ANGLE] = Fadd(object->values[VAL_PARAKOOPA_ANGLE], Frad(FfInt(2L)));
                ...
                case OBJ_BUZZY_SHELL: {
                    ...
                    const fix16_t spd = object->values[VAL_X_SPEED];
                    object->values[VAL_Y_SPEED] = Fadd(object->values[VAL_Y_SPEED], 0x00006186);
                        ...
                        if (object->values[VAL_BRO_THROW_STATE] > 30L) {
                            ...
                            if (missile != NULL)
                                switch (missile->type) {
                                    ...
                                    case OBJ_MISSILE_SILVER_HAMMER: {
                                        missile->values[VAL_MISSILE_OWNER] = oid;
                                        missile->values[VAL_X_SPEED] = FfInt(
                                            (object->flags & FLG_X_FLIP) ? (-1L - (random() % 10L))
                                                                         : (1L + (random() % 10L))
                                        );
                                        missile->values[VAL_Y_SPEED] = FfInt(-6L - (random() % 5L));
                                        missile->flags |= (object->flags & FLG_X_FLIP);
    ...
    while (object_is_alive(oid)) {
        ...
        if (object->flags & FLG_VISIBLE)
            switch (object->type) {
                ...
                case OBJ_MISSILE_SILVER_HAMMER: {
                    draw_object(oid, "R_HAMMRB", FtFloat(object->values[VAL_MISSILE_ANGLE]), WHITE);
                    break;
                }
                ...
                case OBJ_BULLET_BILL: {
                    draw_object(oid, "E_BULLET", 0, WHITE);
                    break;
                }

                case OBJ_SPIKE_CANNON: {
                    draw_object(oid, "E_SBCANN", 0, WHITE);
                    break;
                }

                case OBJ_SPIKE_BALL: {
                    draw_object(oid, "R_SBALL", FtFloat(object->values[VAL_SPIKE_BALL_ANGLE]), WHITE);
                    break;
                }

                case OBJ_SPIKE_BALL_EFFECT: {
                    draw_object(
                        oid, "R_SBALL", FtFloat(object->values[VAL_SPIKE_BALL_ANGLE]),
                        ALPHA(FtInt(object->values[VAL_SPIKE_BALL_ALPHA]))
                    );
                    break;
                }
...
void load_object(GameObjectType type) {
    switch (type) {
        ...
        case OBJ_BILL_BLASTER: {
            ...
            load_object(OBJ_BULLET_BILL);
            load_object(OBJ_EXPLODE);
            break;
        }

        case OBJ_BULLET_BILL: {
            load_texture("E_BULLET");
        ...
        case OBJ_KOOPA: {
            load_texture("E_KOOPAA");
            load_texture("E_KOOPAB");
            load_texture("E_KOOPAC");

            load_texture("E_KOOPRA");
            load_texture("E_KOOPRB");
            load_texture("E_KOOPRC");

            load_sound("STOMP");
            load_sound("KICK");

            load_object(OBJ_KOOPA_SHELL);
            load_object(OBJ_DEAD);
            load_object(OBJ_POINTS);
            break;
        }
        ...
        case OBJ_KOOPA_SHELL: {
            load_texture("E_KOOPAC");
            load_texture("E_KOOPAD");
            load_texture("E_KOOPAE");
            load_texture("E_KOOPAF");

            load_texture("E_KOOPRC");
            load_texture("E_KOOPRD");
            load_texture("E_KOOPRE");
            load_texture("E_KOOPRF");
        ...
        case OBJ_PARAKOOPA: {
            load_texture("E_PKOOPA");
            load_texture("E_PKOOPB");
            load_texture("E_KOOPAC");

            // load_texture("E_PKOORA");
            // load_texture("E_PKOORB");
            load_texture("E_KOOPRC");

            load_sound("STOMP");
            load_sound("KICK");

            load_object(OBJ_KOOPA);
            load_object(OBJ_DEAD);
            load_object(OBJ_POINTS);
            break;
        }
        ...
        case OBJ_SPIKE_CANNON: {
            load_texture("E_SBCANN");

            load_sound("BANG2");

            load_object(OBJ_SPIKE_BALL);
            load_object(OBJ_EXPLODE);
            break;
        }
        ...
        case OBJ_SPIKE_BALL: {
            ...
            load_object(OBJ_SPIKE_BALL_EFFECT);
            break;
        }
        ...
        if (!object_is_alive((ObjectID)index)) {
            ...
            switch (type) {
                ...
                case OBJ_MISSILE_SILVER_HAMMER: {
                    ...
                    object->values[VAL_MISSILE_OWNER] = NULLOBJ;
                    object->values[VAL_MISSILE_HITS] = 2L;
                    break;
                }
                ...
                case OBJ_SPIKE_CANNON: {
                    object->bbox[0][1] = FfInt(22L);
                    object->bbox[1][0] = object->bbox[1][1] = FfInt(64L);
                    break;
                }

                case OBJ_SPIKE_BALL: {
                    object->bbox[0][0] = object->bbox[0][1] = FfInt(-20L);
                    object->bbox[1][0] = object->bbox[1][1] = FfInt(20L);
                    break;
                }

                case OBJ_SPIKE_BALL_EFFECT: {
                    object->values[VAL_SPIKE_BALL_ALPHA] = FfInt(255L);
                    break;
                }
